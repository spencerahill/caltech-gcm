...
! ------------------------------ executable code -----------------------------
  ...
  call get_dimensions(num_lat, num_lon, num_lev, num_times)
  call allocate_and_initialize(num_lat, num_lon, num_lev)
  call axes_init(deg_lat, sin_lat, cos_lat_xy, sin_lat_xy, cos_lat_yz)

! read_surf_geopotential must be called before grid_fields_init
  if(.not.smooth_surface) &
       call read_surf_geopotential(SGVarName, SGInputFileName, geopot_sfc)

  call grid_fields_init( &
       DataIn, &
       is_gaussian, &
       moisture, &
       moist_isentropes, &
  ...
  do while (time < TimeIn + curr_num_times)
     ...
     call read_data( &
          ...
          dt_temp_sw_grid, &
          bucket_depth_grid, &
          bucket_depth_conv_grid, &
          bucket_depth_cond_grid, &
          bucket_depth_lh_grid, &
  ...
  if (moisture) then
     call pot_temp_deriv( &
          ...
          ps_avg, &
          d_dy_pot_temp_e_avg, &
          d_dsig_pot_temp_e_avg, &
          d_dp_pot_temp_e_avg)
  endif

  call residual_circulations( &
       sfctn_avg, &
       mrdnl_eddy_pot_temp_flux_avg, &
       vrtcl_eddy_pot_temp_flux_avg, &
       d_dy_pot_temp_avg, &
       d_dsig_pot_temp_avg, &
       ps_avg, &
       TEM_res_circ, &
  ...
  call write_variables(num_lat, num_fourier, num_lev, time-TimeIn)
...
!                            --- executable code ---
    ...
    vrtcl_v_flux_avg = vrtcl_v_flux_avg  + sigma_avg(v_grid*w_grid, ps_grid)

    vcos_zon_spec_barotr    = vcos_zon_spec_barotr  +                               &
         zonal_spectrum_grid(p_half_zon, vcos_grid)

    dt_u_drag_avg      = dt_u_drag_avg + sigma_avg(dt_u_drag_grid, ps_grid)
    dt_v_drag_avg      = dt_v_drag_avg + sigma_avg(dt_v_drag_grid, ps_grid)

    vort_avg           = vort_avg          + sigma_avg(vor_grid, ps_grid)
    div_avg            = div_avg           + sigma_avg(div_grid, ps_grid)
    ...
    mrdnl_z_deriv_avg    = mrdnl_z_deriv_avg  + sigma_avg(dy_geopot_grid / grav, ps_grid)
...
!!!!!!!!!
    ...
    vrtcl_v_flux_avg     = divide_by_psfc(vrtcl_v_flux_avg/dof,   ps_avg)
    vcos_avg             = divide_by_psfc(vcos_avg/dof,           ps_avg)

    dt_u_drag_avg        = divide_by_psfc(dt_u_drag_avg/dof,      ps_avg)
    dt_v_drag_avg        = divide_by_psfc(dt_v_drag_avg/dof,      ps_avg)

    sfctn_avg            = sfctn_avg            / dof

    vort_avg             = divide_by_psfc(vort_avg/dof,           ps_avg)
    div_avg              = divide_by_psfc(div_avg/dof,            ps_avg)
    ...
    mrdnl_z_deriv_avg    = divide_by_psfc(mrdnl_z_deriv_avg/dof,  ps_avg)
...
!----------------------------- executable code  -------------------------------
    ...
    vrtcl_eddy_v_flux_avg    = vrtcl_v_flux_avg - w_avg * v_avg
